...
The recommended lookup table would be human Ensembl protein IDs to Ensembl gene IDs and HGNC symbols (this last one is optional but highly recommended to add).
...
2. **Lookup table**
Simply import it using the pandas function read_csv. Don't forget to add whatever separator the file has in the *sep* argument ("\t" for tsv files, "," for csv, etc.)
```
...
Again, just a text/tsv/csv file with a single column containing all your curated genes. In our case, human transcription factors. Put a header in the file (some text on the first line, a name for the column) or specify "header = None"
```
>>> query = pd.read_csv("/path/to/TFs_Ensembl_v_1.01.txt", header=None, sep = "\t")
```
...
```
python geneannotator/Orthogroup_argparse.py -g '/path/to/Eggnog_Bilateria(33213)_members.tsv' -g '/path/to/Eggnog_Metazoa(33208)_members.tsv' -l 'path/to/lookup.txt' -q "/path/to/Capitella_emapper_output.txt" -m "Gene stable ID"
```
