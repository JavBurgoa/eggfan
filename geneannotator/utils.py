...
def translate_uniprots(orthotable, lookup):
	...
	for row in list(range(len(orthotable.index))):
		...
		for uniprotID in query:
			...
			genIDs = lookup["ENSEMBL_ID"][lookup["UniProtKB"] == uniprotID] # there are several GenIDs per UniprotID. We will take all of them
...
def directory_or_file(path):
	...
	else: exit("Introduced path is neither a folder or a file")
...
def format_uniprot_output(uniprot, columns):
	uniprot = pd.DataFrame([x.split('\t') for x in uniprot.split('\n')]) # string to DataFrame
...
def lost_genes(TFs, path_to_orthologies, all_lost_genes=False, TF_lost_genes=True):
	...
	for file in os.listdir(path_to_orthologies):
		...
		lostGenes = orthoTable[["GeneName_target", "orthologs"]][orthoTable["GenIDs"].isna()]
	...
	if all_lost_genes:
		...
		finalostable = finalostable.merge(finalist, how="right", left_on="Genes", right_on="Genes")
...
def HGNC_request(gene):
	...
	else: print('Error detected: ' + response['status'])
...
def update_Biomart(lookup, finalostable):
	...
	finalostable = finalostable.merge(lookup2, how="left", left_on="Genes", right_on="HGNC symbol")
...
def find_position(row, gene, column="ENSEMBL_ID", HGNC = False):
	if HGNC:
		...
		for i in position.index.values:
			...
			position[i] = position[i].split("True") # The problem is that if you use regex to split you end up taking the comma aswell, and you cannot just use gene because many HGNCs are substrings of other, so the split gets mistaken. What we are doing is transform into a string of true false and split by True
	
	else: position = row[column].str.split(gene)
	
	position = position.str[0]
	position = position.str.count(",") # if there are 3 "," before the found ID, then the found ID is in the 4th position (python counts from 0, so no need to +1)
...
def query_position_table(rows, position, gene, HGNC = False):
	...
	table = pd.DataFrame({"GenID":[], "HGNC":[], "position_from_0":[], "number_of_IDs":[]})
	...
	for i in indexes:
		...
		number_of_IDs = len(HGNC_symbols[i]) # total number of symbols translated or not
		new = pd.DataFrame({"GenID":[ENS_name], "HGNC":[HGNC_name], "position_from_0":[position_tmp], "number_of_IDs": [number_of_IDs]})
